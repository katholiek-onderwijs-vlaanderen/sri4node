...
function filterString(select, filter, value, mapping, baseType, _field) {
    let values;
    const not = filter.postfix === "Not";
    const sensitive = filter.prefix === "CaseSensitive";
    const tablename = (0, common_1.tableFromMapping)(mapping);
    if ((filter.operator === "Greater" && not && sensitive) ||
        (filter.operator === "Less" && !not && sensitive)) {
        select.sql(` AND ("${tablename}"."${filter.key}"::text COLLATE "C") < `).param(value);
    }
    else if ((filter.operator === "Greater" && !not && sensitive) ||
        (filter.operator === "Less" && not && sensitive)) {
        select.sql(` AND ("${tablename}"."${filter.key}"::text COLLATE "C") > `).param(value);
    }
    else if ((filter.operator === "Greater" && not && !sensitive) ||
...
function filterNumericOrTimestamp(select, filter, value, _mapping, baseType, _field) {
    const isTimestamp = baseType === "timestamp";
    if ((!filter.postfix && filter.operator === "Less") ||
    ...
    else if ((!filter.postfix && filter.operator === "Greater") ||
        ...
        if (filter.postfix === "Not") {
            select
                .sql(` AND ( NOT `)
                .valueIn(isTimestamp ? `${filter.key}::timestamptz` : filter.key, value.split(",").map((v) => parseFunc(v)), isTimestamp ? "timestamptz" : undefined)
                .sql(` OR "${filter.key}" IS NULL )`);
        }
        else {
            select.sql(` AND`).valueIn(filter.key, value.split(",").map((v) => parseFunc(v)), isTimestamp ? "timestamptz" : undefined);
        }
    }
...
function filterBoolean(select, filter, value, _mapping, _baseType, _field) {
    if (value !== "any") {
        if (filter.postfix === "Not") {
            select.sql(" AND NOT ");
        }
...
function filterJson(select, filter, value, mapping, _baseType, _field) {
    const { path } = filter;
    if (path == null) {
        throw new typeDefinitions_1.SriError({
            status: 404,
...
function defaultFilter(valueEnc, query, parameter, _tx, _doCount, mapping, _urlParameters) {
    ...
    const { informationSchema } = global.sri4node_configuration;
    const idx = mapping.type;
    const field = (_a = informationSchema === null || informationSchema === void 0 ? void 0 : informationSchema[idx][filter.key]) !== null && _a !== void 0 ? _a : null;
    ...
    if (field) {
        ...
        filterGeneral(query, value, getTextFieldsFromTable(informationSchema === null || informationSchema === void 0 ? void 0 : informationSchema[idx]));
    }
    else {
        throw new typeDefinitions_1.SriError({
            status: 404,
            errors: [
                {
                    ...
                    parameter,
                    possibleParameters: Object.keys((_b = informationSchema === null || informationSchema === void 0 ? void 0 : informationSchema[idx]) !== null && _b !== void 0 ? _b : {}),
                },
            ],
        });
    }
