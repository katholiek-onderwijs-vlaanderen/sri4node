...
describe('Sri4node testing', function () {
  ...
  this.timeout(0);
    ...
    it('batch with lot of items (chunked) - not allowed by security', async function () {
        const batch = fs.readFileSync('test_performance/curl-delete-customcur-outOfMemory_chunked.json');
        const startTime = Date.now();
        const {
          trailers,
          body
        } = await client.request({ path: '/llinkid/customcurriculum/customcurricula/batch_streaming?dryRun=true'
                          , method: 'POST'
                          , headers: {
        ...
        const accServerTiming = trailers['server-timing'].split(',').reduce( (acc, st) => acc + parseInt(st.split(';dur=')[1]), 0 );
