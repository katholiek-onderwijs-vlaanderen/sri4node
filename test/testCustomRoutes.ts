  ...
  describe("Custom routes", () => {
    it("should return the response for the custom route", async () => {
      const response = await httpClient.get({
        path: "/persons/de32ce31-af0c-4620-988e-1d0de282ee9d/simple",
        auth: "kevin",
      });
    ...
    it("should forbid the custom route because of a secure function", async () => {
      const response = await httpClient.get({
        path: "/persons/da6dcc12-c46f-4626-a965-1a00536131b2/simple",
        auth: "ingrid",
      });
    ...
    it("should return a server error for a problematic custom handler", async () => {
      const result = await httpClient.get({
        path: "/persons/00000000-0000-0000-0000-000000000000/simple",
        auth: "kevin",
      });
    ...
    it("should return the response for the custom 'like' route with alterMapping", async () => {
      const response = await httpClient.get({
        path: "/persons/de32ce31-af0c-4620-988e-1d0de282ee9d/simpleLike",
        auth: "kevin",
      });
    ...
    it("should return the response for the custom 'like' route with transformResponse", async () => {
      const response = await httpClient.get({
        path: "/persons/de32ce31-af0c-4620-988e-1d0de282ee9d/simpleLike2",
        auth: "kevin",
      });
      assert.equal(Object.keys(response.body).length, 2);
      assert.equal(response.body.firstname, "Kevin");
    ...
    it("should return the response for the custom 'like' route for forbidden user in 'original' route", async () => {
      await httpClient.get({
        path: "/persons/da6dcc12-c46f-4626-a965-1a00536131b2/simpleLike",
        auth: "ingrid",
      });
    });
    ...
    it("streamingHandler JSON stream should work", async function () {
      ...
      const { status, headers, body } = await httpClient.get({
      ...
      assert.isBelow(
        duration,
        500,
        `Streaming request took more than 500ms (${duration}) before starting to stream !`,
      );
    }).timeout(4000);
    ...
    it("streaming file upload (multipart form) should work", async () => {
      ...
      const p = await httpClient.get({
        path: "/persons/ab0fb783-0d36-4511-8ca5-9e29390eea4a",
        auth: "eddy",
      });
    ...
    it("streaming custom route: status and headers set via beforeStreamingHandler should be returned to client", async () => {
      const { status, headers } = await httpClient.post({
        path: "/persons/test_before_streaming_handler",
        body: {},
        auth: "kevin",
        streaming: true,
      });
      ...
      assert.equal(
        headers["MyTestHeader".toLowerCase()],
        "MyTestValue",
    ...
    it("streaming custom route: error handling of error in streamingHandler", async () => {
      ...
      assert.equal(
        partialBody.includes('"status": 400'),
        true,
        "expected status code 400 missing in partial body",
      );
      assert.equal(
        partialBody.includes('"code": "customroute.bad.request"'),
        true,
        "expected error code missing in partial body",
      );
    });
