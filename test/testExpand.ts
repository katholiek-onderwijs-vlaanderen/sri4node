...
module.exports = function (httpClient: THttpClient) {
  ...
  describe('Expansion', () => {
    ...
    describe(' with "none" on list resources with \'addReferencingResources\' configured', () => {
      it('should succeed without $$expanded in results array.', async () => {
        const response = await httpClient.get({ path:'/persons?expand=none&communities=/communities/8bf649b4-c50a-4ee9-9b02-877aa0a71849', auth: 'sabine' });
        assert.equal(response.status, 200);
        if (response.body.results[0].$$expanded) {
          assert.fail('Expansion was performed !');
        }
    ...
    describe('on regular resources', () => {
      it('should succeed with $$expanded as result.', async () => {
        const response = await httpClient.get({ path:'/messages/ad9ff799-7727-4193-a34a-09f3819c3479?expand=community', auth: 'sabine' });
        assert.equal(response.status, 200);
      ...
      it('should fail due to secure function on expanded resource', async () => {
        const response = await httpClient.get({ path:'/messages/7f5f646c-8f0b-4ce6-97ce-8549b8b78234?expand=person', auth: 'sabine' });
        assert.equal(response.status, 403);
      });

      it('should succeed since the expanded resource is valid', async () => {
        const response = await httpClient.get({ path:'/messages/5a2747d4-ed99-4ceb-9058-8152e34f4cd5?expand=person', auth: 'kevin' });
        assert.equal(response.status, 200);
      });
      ...
      it('should fail since 1 of the expandeded chained resources will throw due to an afterRead function in communities', async () => {
        const response = await httpClient.get({ path:'/messages/5a2747d4-ed99-4ceb-9058-8152e34f4cd5?expand=person.community', auth: 'kevin' });
        assert.equal(response.status, 403);
      });
    });
    ...
    describe('on list resources', () => {
      it('should succeed with $$expanded as result.', async () => {
        const response = await httpClient.get({ path:'/messages?limit=3&expand=results.community', auth: 'sabine' });
        assert.equal(response.status, 200);
      ...
      it('should fail due to secure function on expanded resource', async () => {
        const response = await httpClient.get({ path:'/messages?expand=results.person', auth: 'sabine' });
        assert.equal(response.status, 403);
      });

      it('should succeed since the expanded resource is valid', async () => {
        const response = await httpClient.get({ path:'/messages?titleContains=Vervoer&expand=results.person', auth: 'kevin' });
        assert.equal(response.status, 200);
      });

      it('should fail since 1 of the expandeded chained resources will throw due to an afterRead function in communities', async () => {
        const response = await httpClient.get({ path:'/messages?titleContains=Vervoer&expand=results.person.community', auth: 'kevin' });
        assert.equal(response.status, 403);
      });
    });
    ...
    describe('with invalid', () => {
      it('should say \'not found\'.', async () => {
        const response = await httpClient.get({ path:'/messages/ad9ff799-7727-4193-a34a-09f3819c3479?expand=invalid', auth: 'sabine' });
    ...
    describe('on list resources', () => {
      it('should allow expanding multiple keys.', async () => {
        const response = await httpClient.get({ path:'/messages?limit=3&expand=results.person,results.community', auth: 'sabine' });
        assert.equal(response.status, 200);
        if (response.body.results[0].$$expanded.community.$$expanded === null) {
          assert.fail('Expansion was not performed !');
        }
    ...
    describe('with 2 level path (x.y)', () => {
      it('should expand recursively.', async () => {
        const response = await httpClient.get({ path:'/messages?limit=3&expand=results.person.community,results.community', auth: 'sabine' });
        assert.equal(response.status, 200);
        if (response.body.results[0].$$expanded.community.$$expanded === null
          || response.body.results[0].$$expanded.community.$$expanded === undefined) {
          assert.fail('Expansion was not performed !');
    ...
    describe(' on missing non-mandatory property', () => {
      it(' should result in null value for null value.', async () => {
        try {
          const response = await httpClient.get({ path:'/store/products?expand=results.package2', auth: 'sabine' });
          assert.equal(response.status, 200);
      ...
      it(' should result in null value for undefined.', async () => {
        const response = await httpClient.get({ path:'/store/products?expand=results.package3', auth: 'sabine' });
        assert.equal(response.status, 200);
