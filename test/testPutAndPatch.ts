...
module.exports = function (httpClient: THttpClient) {
  ...
  describe("PUT", () => {
    describe("schema validation", () => {
      ...
      it("should support complex schema's using oneOf/anyOf/allOf", async function () {
        ...
        assert.equal(
          response1.status,
          409,
          "no extra properties should be allowed according to the json-schema",
        );
        ...
        assert.equal(
          response4.status,
          200,
          "creation of a valid resource with foo as a string failed",
        );
        ...
        assert.isUndefined(response4get.body.bar, "body not as expected");
        ...
        assert.equal(
          response5.status,
          200,
          "creation of a valid resource with foo as a number failed",
        );
    ...
    describe("with a missing field (community without name)", () => {
      it("should return a 409 Conflict", async () => {
        ...
        assert.equal(response.status, 409);
        assert.equal(
          response.body.errors[0].errors.validationErrors[0].code,
    ...
    describe("with a numeric value of 0", () => {
      it("should work and not skip 0 as a null value", async () => {
        ...
        const body = generateTransaction(
          ...
          "/persons/9abe4102-6a29-4978-991e-2a30655030e6",
          0,
        );
  ...
  describe("VALIDATION", () => {
    describe("schema validation", () => {
      it("should detect if a field is too long", async () => {
        ...
        const response = await httpClient.put({
          path: `/communities/${key}?dryRun=true`,
          body,
          auth: "sabine",
        });
        assert.equal(response.status, 409);
    ...
    describe("with a missing field (community without name)", () => {
      it("should return a 409 Conflict", async () => {
        ...
        assert.equal(
          result.body.errors[0].errors.validationErrors[0].code,
          "must.have.required.property.name",
        );
      });
    });
    ...
    describe("with a numeric value of 0", () => {
      it("should work and not skip 0 as a null value", async () => {
        ...
        const response = await httpClient.put({
          path: `/transactions/${key}?dryRun=true`,
          body,
          auth: "sabine",
        });
        assert.equal(response.status, 201);
  ...
  describe("key in PUT ", () => {
    ...
    it("should return error for invalid UUID", async () => {
      ...
      assert.equal(
        response.body.errors[0].errors.validationErrors[0].code.substring(0, 22),
        "must.match.pattern",
      );
    });
  });
  ...
  describe("PATCH", () => {
        ...
        const p = [
          {
            op: "replace",
            path: "/community/href",
            value: "/communities/00000000-0000-0000-0000-000000000000",
          },
        ];
    ...
    describe("simple working PATCH", () => {
      ...
      it("should be idempotent", async () => {
        ...
        const responsePatch1 = await httpClient.patch({
          path: personSabine,
          body: patch,
          auth: "sabine",
        });
        ...
        const responsePatch2 = await httpClient.patch({
          path: personSabine,
          body: patch,
          auth: "sabine",
        });
    ...
    describe("PATCH causing schema failures", () => {
      it("should return 409 conflict because schema will fail", async () => {
        const patch = [{ op: "replace", path: "/community/href", value: "INVALID" }];
        const response = await httpClient.patch({
          path: personSabine,
          body: patch,
          auth: "sabine",
