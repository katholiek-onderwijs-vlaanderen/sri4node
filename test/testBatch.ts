...
module.exports = function (httpClient: THttpClient) {
  ...
  describe("BATCH", () => {
    ...
    it("2 consecutive PATCHes on existing person", async () => {
      ...
      assert.equal(responsePut2.status, 200);
      responsePut2.body.forEach((x) =>
        x.status ? assert.equal(x.status, 200) : x.forEach((x) => assert.equal(x.status, 200)),
      );

      const responseGet = await httpClient.get({ path: personHref, auth: "sabine" });
      assert.equal(responseGet.body.key, key);
      assert.equal(
        responseGet.body.streetnumber,
        newStreetNumber,
        "seems like the second patch operation on streetbus undid the previous patch on streetnumber",
      );
    ...
    it("with error should be completely rollbacked", async () => {
      ...
      const reponsePut = await httpClient.put({
        path: "/communities/batch",
        body: batch,
        auth: "sabine",
      });
      ...
      assert.equal(
        reponsePut.body[1].body.errors[0].errors.validationErrors[0].code,
    ...
    it("with error should be completely rollbacked", async () => {
      ...
      assert.equal(
        responsePut.body[1].body.errors[0].errors.validationErrors[0].code,
        "must.have.required.property.name",
      );
    ...
    it("error should result in cancellation of accompanying requests ", async () => {
      ...
      assert.equal(
        response.body[0].body.errors[0].errors.validationErrors[0].code,
        "must.have.required.property.name",
      );
    ...
    it("batch with multi row insert and a constraint error", async () => {
      ...
      assert.strictEqual(
        response.body[2].href,
        "/persons/de32ce31-af0c-4620-988e-1d0de282ee9d/simple",
      );
    });
