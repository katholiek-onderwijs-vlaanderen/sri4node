...
import { URL } from 'url';
import { FlattenedJsonSchema, ParseTree, ResourceDefinition, SriConfig, SriRequest } from '../js/typeDefinitions';
...
const { generateFlatQueryStringParserGrammar } = require('../js/url_parsing/flat_url_parser')
const { generateNonFlatQueryStringParserGrammar } = require('../js/url_parsing/non_flat_url_parser')
...
// Therefore we cannot use a strict uuid checker like the npm module 'uuid-validate' but do we have to be less strict.
    ...
    Object.keys(map).forEach( key => {
      if (map[key].references && obj[key] !== undefined) {
        ...
        if (refType === expectedType) {
          ...
          module.exports.cl(msg);
          throw new module.exports.SriError({status: 409, errors: [{code: 'faulty.reference', msg: msg}]})
        }
      } else {
  ...
  pgConnect: async function (arg) {
    ...
    if (dbUrl === undefined) {
        module.exports.error('FATAL: database configuration is missing !');
        process.exit(1)
    }
  ...
  startTransaction: async (db, sriRequest=null, mode = new pgp.txMode.TransactionMode()) => {
    const hrstart = process.hrtime();
    module.exports.debug('db', '++ Starting database transaction.');
    ...
    const txWrapper = async (emitter) => {
      ...
      //   * module.exports.debug() does not log the correct reqId
      try {
    ...
    try {
      ...
      module.exports.debug('db', 'Got db tx object.');
      ...
      module.exports.error('CAUGHT ERROR: ');
      module.exports.error(JSON.stringify(err));
      throw new module.exports.SriError({status: 503, errors: [{code: 'too.busy', msg: 'The request could not be processed as the database is too busy right now. Try again later.'}]})
    }
  },
  ...
  startTask: async (db, sriRequest=null) => {
    const hrstart = process.hrtime();
    module.exports.debug('db', '++ Starting database task.');
    ...
    try {
      ...
      module.exports.debug('db', 'Got db t object.');

      const endTask = async () => {
          emitter.emit('terminate')
          const res = await pEvent(emitter, 'tDone')
          module.exports.debug('db', 'db task done.');
          if (res !== undefined) {
            throw res
          }
      }
  ...
  settleResultsToSriResults: (results) => {
    return results.map(res => {
        if (res.isFulfilled) {
          ...
          if (err instanceof module.exports.SriError) {
            return err
          } else {
            module.exports.error('____________________________ E R R O R (settleResultsToSriResults)_________________________')
            module.exports.error(exports.stringifyError(err))
            if (err && err.stack) {
                module.exports.error('STACK:')
                module.exports.error(err.stack)
            }
            module.exports.error('___________________________________________________________________________________________')
            return new module.exports.SriError({status: 500, errors: [{code: 'internal.server.error', msg: `Internal Server Error. [${exports.stringifyError(err)}]`}]})
          }
        }
      });
  },
