    ...
    describe("Global hooks", () => {
      ...
      describe("beforePhase & afterRequest", () => {
        ...
        it("single request", async () => {
          ...
          // patch
          dummyLoggerSpy.resetHistory();
          const patch = [{ op: "replace", path: "/streetnumber", value: "25" }];
          ...
          dummyLoggerSpy.resetHistory();
          await httpClient.get({
            path: "/communities/customroute_via_internal_interface",
            auth: "kevin",
          });
          assert.equal(
            dummyLoggerSpy.calledWith(`final beforePhaseCntr: ${nrPhasesInRequest * 2}`),
            true,
          );
        });
        ...
        it("batch request - sequential", async () => {
          ...
          await httpClient.put({ path: "/batch", body: batch, auth: "kevin" });
          assert.equal(
            dummyLoggerSpy.calledWith(`final beforePhaseCntr: ${nrPhasesInRequest * 8}`),
            true,
          );
        });
    ...
    describe("Resource specific hooks", () => {
      describe("before and after hooks (CRUD)", async () => {
        ...
        it("read", async () => {
          // single
          await httpClient.get({
            path: "/bars/5de9c352-2534-11ed-84bc-9bce6d5e13f9",
            auth: "kevin",
          });
          // list
        ...
        it("update & patch", async () => {
          ...
          await httpClient.put({ path: `/bars/${testKey}`, body, auth: "kevin" });
          // patch
          const patch = [{ op: "replace", path: "/foo", value: "patch" }];
            ...
            [
              {
                ...
                href: `/bars/${testKey2}`,
                verb: "PATCH",
                body: [{ op: "replace", path: "/foo", value: "patch" }],
              },
            ],
            [
              {
        ...
        describe("via internal interface", async () => {
          it("non-batch", async () => {
            ...
            // list
            await httpClient.post({
              path: "/bars/proxy_internal_interface?method=GET&href=/bars",
              auth: "kevin",
            });
            // create
            await httpClient.post({
              path: `/bars/proxy_internal_interface?method=PUT&href=/bars/${testKey2}`,
              body: {
                key: testKey2,
                foo: "create",
              },
            ...
            // patch
            await httpClient.post({
              path: `/bars/proxy_internal_interface?method=PATCH&href=/bars/${testKey2}`,
              body: [{ op: "replace", path: "/foo", value: "patch" }],
              auth: "kevin",
            });
            // delete existing
            await httpClient.post({
              path: `/bars/proxy_internal_interface?method=DELETE&href=/bars/${testKey2}`,
              auth: "kevin",
            });
            // delete already deleted
            await httpClient.post({
            ...
            // custom
            await httpClient.post({
              path: `/bars/proxy_internal_interface?method=GET&href=/onlyCustom`,
              auth: "kevin",
            });
          });
          ...
          it.skip("batch", async () => {
            ...
            await httpClient.post({
              path: `/bars/proxy_internal_interface?method=PUT&href=/batch`,
              body: batch,
              auth: "kevin",
            });
          });
