...
module.exports = function (httpClient: THttpClient) {
  ...
  async function applyFunctionToList(
    asyncFunctionToApply: (
      apiResponse: Array<string | Record<string, unknown>>,
      lastPage: boolean,
      pageNum: number,
      countBeforeCurrentPage: number,
    ) => any,
    url: string,
    options: Record<string, unknown> = {},
  ) {
    let nextPath: string | null = url;
    let nextJsonDataPromise: Promise<THttpResponse> | null = httpClient.get({
      path: url,
      ...options,
    });
    let pageNum = 0;
  ...
  describe("GET public list resource", function () {
    ...
    describe("with single value ?hrefs=...", function () {
      it("should work", async function () {
        const response = await httpClient.get({
          path: "/communities?hrefs=/communities/1edb2754-8481-4996-ae5b-ec33c903ee4d",
          auth: "sabine",
        });
        assert.equal(response.body.$$meta.count, 1);
        assert.equal(
          response.body.results[0].href,
          "/communities/1edb2754-8481-4996-ae5b-ec33c903ee4d",
        );
      });
    });
    ...
    describe("with two values ?hrefs=...", function () {
      it("should work", async function () {
        const response = await httpClient.get({
          path:
            "/communities?hrefs=/communities/1edb2754-8481-4996-ae5b-ec33c903ee4d" +
  ...
  describe("GET private list resource", function () {
    ...
    describe("/persons with authentication", function () {
      it("should be 200 Ok", async function () {
        ...
        const response = await httpClient.get({
          path: "/persons?communities=/communities/8bf649b4-c50a-4ee9-9b02-877aa0a71849",
          auth: "sabine",
        });
  ...
  describe("Paging", function () {
    ...
    it("should use key of last value in the resultlist as key offset (descending)", async function () {
      const response = await httpClient.get({
        path: "/alldatatypes?descending=true",
        auth: "kevin",
      });
      assert.equal(
        response.body.$$meta.next.endsWith(
    ...
    it("should use key of last value in the resultlist as key offset (ascending)", async function () {
      const response = await httpClient.get({
        path: "/alldatatypes?descending=false",
        auth: "kevin",
      });
      assert.equal(
        response.body.$$meta.next.endsWith(
    ...
    it("should work when expand=NONE is combined with orderBy (only using properties that are never NULL)", async function () {
      ...
      await applyFunctionToList(
        (apiResponse, isLastPage, _pageNum, countBeforeCurrentPage) => {
          ...
          if (isLastPage) {
            assert.equal(
              countBeforeCurrentPage + (apiResponse as any).results.length,
              (apiResponse as any).$$meta.count,
            );
          }
        },
      ...
      await applyFunctionToList(
        (apiResponse, _isLastPage, _pageNum, _countBeforeCurrentPage) => {
          ...
          assert.notStrictEqual(
            (apiResponse as any).results?.length,
            0,
            "we got a response with a results array containing zero elements",
          );
        },
    ...
    it("should allow unlimited resources with expand none", async function () {
      const response = await httpClient.get({
        path: "/alldatatypes?limit=*&expand=none",
        auth: "kevin",
      });
    ...
    it("should propagate parameters in the next page", async function () {
      const response = await httpClient.get({
        path: "/alldatatypes?textContains=a&limit=2",
        auth: "kevin",
      });
      assert.equal(response.body.results.length, 2);
      assert.equal(
        response.body.$$meta.next.startsWith("/alldatatypes?textContains=a&limit=2&keyOffset"),
        true,
      );
    });
    ...
    it("should be able to deal with limit=0 and no results", async function () {
      const response = await httpClient.get({
        path: "/persons?firstname=unexisting&limit=0",
        auth: "sabine",
      });
  ...
  describe("Count ", function () {
    ...
    it("should be present when explicitely requested", async function () {
      const response = await httpClient.get({
        path: "/alldatatypes?$$includeCount=true",
        auth: "kevin",
      });
      assert.equal(response.body.$$meta.count !== undefined, true);
    });
    ...
    it("should be omitted when explicitely requested", async function () {
      const response = await httpClient.get({
        path: "/alldatatypes?$$includeCount=false",
        auth: "kevin",
      });
      assert.equal(response.body.$$meta.count === undefined, true);
    });
    ...
    it("should be present when explicitely requested (resource with includeCount=false as default)", async function () {
      const response = await httpClient.get({
        path: "/messages?$$includeCount=true",
        auth: "sabine",
      });
      assert.equal(response.body.$$meta.count !== undefined, true);
    });
    ...
    it("should be omitted when explicitely requested (resource with includeCount=false as default)", async function () {
      const response = await httpClient.get({
        path: "/messages?$$includeCount=false",
        auth: "sabine",
      });
      assert.equal(response.body.$$meta.count === undefined, true);
    });
  ...
  describe("Custom query parameter", function () {
    it("should return all resources", async function () {
      ...
      const response2 = await httpClient.get({
        path: "/personrelations?status=ABOLISHED",
        auth: "sabine",
      });
      assert.equal(response2.body.results.length, 1);
      const response3 = await httpClient.get({
        path: "/personrelations?status=ACTIVE",
        auth: "sabine",
      });
    ...
    it("error", async function () {
      const response = await httpClient.get({
        path: "/personrelations?status=foo",
        auth: "sabine",
      });
      assert.equal(response.status, 409);
    ...
    it("unkown query parameter", async function () {
      const response = await httpClient.get({
        path: "/personrelations?unkown_para=55",
        auth: "sabine",
      });
      assert.equal(response.status, 404);
