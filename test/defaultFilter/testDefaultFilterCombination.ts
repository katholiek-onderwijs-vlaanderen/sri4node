  ...
  describe('Generic Filters', () => {
    describe('Combination match', () => {
      it('should find resources with a combined match', async () => {
        const response = await httpClient.get({ path:'/alldatatypes?text=vsko&number=450', auth: 'kevin' });
      ...
      it('should find resources with a combined match and modifiers', async () => {
        const response = await httpClient.get({ path:'/alldatatypes?textCaseSensitiveNot=VSKO&numberAfter=230', auth: 'kevin' });
        assert.equal(response.status, 200);
      ...
      it('should not find resources with a combined match', async () => {
        const response = await httpClient.get({ path:'/alldatatypes?textCaseSensitive=vsko&number=230', auth: 'kevin' });
        assert.equal(response.status, 200);
