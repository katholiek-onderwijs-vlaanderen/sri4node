...
function generateFlatQueryStringParserGrammar(flattenedJsonSchema) {
  ...
  const grammar = `
    { //////// START OF JAVASCRIPT FUNCTIONS BLOCK ////////
      ...
      const multiValuedProperties = new Set([${allMultiValuedPropertyNamesSortedInReverse
        .map((n) => `'${n}'`)
        .join()}]);
      ...
      const propertyNameToOtherThanStringTypeMap = ${JSON.stringify(
        propertyNameToOtherThanStringTypeMap,
      )}
    ...
    MultiValuedProperty = ${
      hasMultiValuedProperties
        ? allMultiValuedPropertyNamesSortedInReverse.map((n) => `"${n}"`).join(" / ")
        : '"\\\\"'
    }

    SingleValuedProperty = ${
      hasSingleValuedProperties
        ? allSingleValuedPropertyNamesSortedInReverse.map((n) => `"${n}"`).join(" / ")
        : '"\\\\"'
    }
  `;
