...
function generateNonFlatQueryStringParserGrammar(
  ...
  const grammar = `
    ...
    ListControlParameter
      ...
      // translate $$includeCount to LIST_INCLUDE with property value $$meta.count (${encodeURIComponent(
        "$$",
      )}meta.count)
    ...
    ListControlOperator = 
      ( "_LIST_LIMIT" ${
        !allPropertyNamesSorted.includes("limit") ? '/ "limit"' : ""
      } ) { return { name: 'LIST_LIMIT', type: 'integer', multiValued: false } }
      / ( "_EXPANSION" ${
        !allPropertyNamesSorted.includes("expand") ? '/ "expand"' : ""
      } ) { return { name: 'EXPANSION', type: 'string', multiValued: true } }
      / ( "_LIST_OFFSET" ${
        !allPropertyNamesSorted.includes("offset") ? '/ "offset"' : ""
      } ) { return { name: 'LIST_OFFSET', type: 'integer', multiValued: false } }
      / ( "_LIST_KEYOFFSET" ${
        !allPropertyNamesSorted.includes("keyOffset") ? '/ "keyOffset"' : ""
      } )  { return { name: 'LIST_KEY_OFFSET', type: 'string', multiValued: true } }
      / ( "_LIST_META_INCLUDE" ${
        !allPropertyNamesSorted.includes("%24%24includeCount") ? '/ "%24%24includeCount"' : ""
      } ) { return { name: 'LIST_META_INCLUDE_COUNT', type: 'boolean', multiValued: false } }
      ...
      // / ( "_LIST_PROPERTY_EXCLUDE" ${
        !allPropertyNamesSorted.includes("omit") ? '/ "omit"' : ""
      } ) { return { name: 'LIST_PROPERTY_EXCLUDE', type: 'boolean', multiValued: false } }
      / ( "_LIST_ORDER_BY" ${
        !allPropertyNamesSorted.includes("orderBy") ? '/ "orderBy"' : ""
      } ) { return { name: 'LIST_ORDER_BY', type: 'string', multiValued: true } }
      / ( "_LIST_ORDER_DESCENDING" ${
        !allPropertyNamesSorted.includes("descending") ? '/ "descending"' : ""
      } ) { return { name: 'LIST_ORDER_DESCENDING', type: 'boolean', multiValued: false } }
    ...
    Property = ${allPropertyNamesSortedInReverse
      .map((n) => {
        ...
        return `p:"${encodeURIComponent(n)}" { return { name: '${decodeURIComponent(
          n,
        )}', type: '${type}', multiValued: ${n.endsWith("[*]")} } }`;
      })
      .join(" / ")}
